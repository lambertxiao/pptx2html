...
export class HtmlDrawer implements Drawer {
  ...
  drawPicNode(node: PicNode) {
    ...
    let html = `
      <div class="block content" style="${styles}" _name="${node.name}">
  ...
  drawShapeNode(node: ShapeNode): string {
    ...
    if (node.bgImg) {
      styles += `background-image: url(${node.bgImg}); background-size: cover;`
      fillColor = "none"
    }

    let html = `<svg class="drawing" style="${styles}" _name="${node.name}">`
  ...
  drawTextNode(node: TextNode): string {
    ...
    let html = `<div class="block" style="${strStyles}" _name="${node.name}">`
